...
class Animation {
  ...
  toFrame(frame, row = this.row) {
    if (!(this.startFrame >= frame <= this.lastFrame)) {
      throw (
        "Frame " +
        frame +
        " solicitado, mas anima��o possui apenas frames " +
        this.startFrame +
        "-" +
        this.lastFrame +
        "!"
  ...
  addFrames(frames, tolerant = true) {
    if (tolerant) {
      if (this.frame + frames < this.startFrame) {
        return this.toFrame(
          this.qtdFrames - ((this.frame + frames) % this.qtdFrames)
        );
  ...
  prevTimedFrame(tolerant = true) {
    if (game.tick % parseInt(this.ticksForAFrame) == 0)
      return this.prevFrame(tolerant);
    else return this;
  }
  nextTimedFrame(tolerant = true) {
    if (game.tick % parseInt(this.ticksForAFrame) == 0)
      return this.nextFrame(tolerant);
    else return this;
  }
  draw(
    dest_x = this.dest_x,
    ...
    ctx.drawImage(
      this.image, // Image
      this.src_x,
      this.src_y, // Source X/Y
      this.src_width,
      this.src_height, // Source Width/Height
      dest_x - dest_width / 2,
      dest_y - dest_height / 2, // Destination X/Y
      dest_width,
      dest_height // Destination Width/Height
    );
    return this;
  }
}
...
class ProgressBar {
  ...
  setValue(value) {
    if (value > this.maxProgress) {
      ...
      this.currProgressPerc =
        (value - this.minProgress) / (this.maxProgress - this.minProgress);
    }
  }
  setPerc(percentual) {
    if (percentual > 1) {
      ...
      this.currProgress =
        percentual * (this.maxProgress - this.minProgress) + this.minProgress;
  ...
  draw() {
    ...
    ctx.drawImage(
      ...
      0, // Source X/Y
      168,
      138, // Source Width/Height
      drawX + this.dW - 17,
      drawY - this.dH / 2, // Destination X/Y
      34,
      27 // Destination Width/Height
    );
    ctx.drawImage(
      ...
      0, // Source X/Y
      197,
      140, // Source Width/Height
      drawX + this.currProgressPerc * this.dW - 20,
      drawY - 28 / 2 + this.dH / 2, // Destination X/Y
      39,
      28 // Destination Width/Height
    );
  }
}
  ...
  draw() {
    ...
    function printIcon(iconIndex, lifeBar, measures) {
      ...
      ctx.drawImage(
        ...
        0, // Source X/Y
        168,
        144, // Source Width/Height
        measures.drawX + lifeBar.icoW * (iconIndex - 1),
        measures.drawY, // Destination X/Y
        lifeBar.icoW,
        lifeBar.dH // Destination Width/Height
      );
    }
...
const help1 = new Animation(
  (imageSrc = "./images/GUI/clique.png"),
  (src_width = 400),
  (src_height = 485),
  (row = 0),
  (startFrame = 0),
  (qtdFrames = 2),
  (repeat = true),
  (dest_x = canvas_WIDTH / 2),
  (dest_y = canvas_HEIGHT / 2),
  (dest_width = 400 / 5),
  (dest_height = 485 / 5),
  (ticksForAFrame = 30)
);
...
class Player {
  constructor() {
    ...
    this.anim = {
      idle: new Animation(
        (imageSrc = "./images/Sprites/Idle_24.png"),
        (src_width = 343),
        (src_height = 248),
        (row = 0),
        (startFrame = 0),
      ...
      walking: new Animation(
        (imageSrc = "./images/Sprites/walk_24.png"),
        (src_width = 345),
        (src_height = 259),
        (row = 0),
        (startFrame = 0),
      ...
      running: new Animation(
        (imageSrc = "./images/Sprites/run_9.png"),
        (src_width = 351),
        (src_height = 261),
        (row = 0),
        (startFrame = 0),
      ...
      jumping_up: new Animation(
        (imageSrc = "./images/Sprites/jump_24.png"),
        (src_width = 350),
        (src_height = 317),
        (row = 0),
        (startFrame = 0),
      ...
      flying: new Animation(
        (imageSrc = "./images/Sprites/fly_14.png"),
        (src_width = 384),
        (src_height = 354),
        (row = 0),
        (startFrame = 0),
      ...
      dying: new Animation(
        (imageSrc = "./images/Sprites/die_14.png"),
        (src_width = 357),
        (src_height = 259),
        (row = 0),
        (startFrame = 0),
...
class Layer {
  constructor(imageSrc, vX_layerModifier, level_speed) {
    ...
    this.vX =
      level_speed *
      (this.vX_layersDefault + this.vX_layerModifier * this.vX_layersDefault);
    // Dimens�es
...
class Level {
  constructor(
    ...
    backgrounds.forEach((bg) => {
      ...
      if (lrLen < bgLen)
        vX_layerModifier = -0.5 - (0.4 * (bgLen - lrLen)) / (bgLen - 1);
      else vX_layerModifier = 0;
      layers.push(
        new Layer(res_path + "/" + bg, vX_layerModifier, start_speed)
      );
    });
    this.layers = layers;
    //PLAYER
  ...
  update_obstacles(meta = true, draw = true) {
    if (this.obstacles.length + this.waitingObstacles.length < 4) {
      this.waitingObstacles.push(
        this.tick + Math.floor(Math.random() * 2000) + 300
      );
    }
    let tick = this.tick;
    let obstacles = this.obstacles;
    this.waitingObstacles.forEach(function (
      item,
      index,
      object,
      tick,
...
var levels = {
  lv1: new Level(
    (name = "Fase 1"),
    (res_path = "./images/levels/lv1"),
    (type = "endless-running-side-2d"),
...
class Obstaculo {
  constructor() {
    ...
    this.anim = {
      idle: new Animation(
        (imageSrc = "./images/Obstaculos/trap miss_13.png"),
        (src_width = 411),
        (src_height = 273),
        (row = 0),
        (startFrame = 0),
      ...
      closing: new Animation(
        (imageSrc = "./images/Obstaculos/trap hit_4.png"),
        (src_width = 411),
        (src_height = 254),
        (row = 0),
        (startFrame = 0),
  ...
  update(speed) {
    ...
    if (
      (distance < this.radius + game.level.player.radius) &
      (this.state == "idle")
    ) {
  ...
  next_tick() {
    ...
    switch (this.state) {
      ...
      case "level_running":
        ...
        if (this.level.tick % 100 == 0)
          console.log(
            this.level.tick,
            this.level.waitingObstacles,
      ...
      case "game_over":
        ...
        ctx.fillText(
          "sua pontuação é de " + this.level.player.score,
          canvas.width / 2 - 60,
          canvas.height / 2 + 20
        );
